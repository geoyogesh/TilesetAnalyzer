...
from tileset_analyzer.data_source.mbtiles.sql_queries import (
    SQL_ALL_TILES,
    SQL_AVG_TILE_SIZES_BY_Z,
    SQL_COUNT_TILES,
    SQL_COUNT_TILES_BY_Z,
    SQL_LIST_TILE_SIZES_BY_Z,
    SQL_MAX_TILE_SIZES_BY_Z,
    SQL_MIN_TILE_SIZES_BY_Z,
    SQL_SUM_TILE_SIZES_BY_Z,
)
...
from tileset_analyzer.entities.job_param import CompressionType, JobParam
...
class MBTilesSource(TileSource):
    ...
    def _get_agg_tile_size_percentiles_z(self, percentile_type: str) -> list[LevelSize]:
        if percentile_type == "50p":
            ...
            raise ValueError("UNKNOWN PERCENTILE TYPE")
    ...
    def _preprocess_tile_layer_sizes(self):
        ...
        def normalize_tile_sizes(total: int, layers: dict[str, int]) -> dict[str, int]:
            final = {}
            layer_sum = sum(list(layers.values()))
        ...
        def process_tile(tile):
            ...
            level_tile_sizes.append(
                TileItemSize(tile.x, tile.y, tile.z, size, tile_all_layers_size)
            )
