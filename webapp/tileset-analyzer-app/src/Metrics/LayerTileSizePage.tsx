...
const LayerTileSize: FC = () => {
  const [tilesSizeAggbyZLayer, setTilesSizeAggbyZLayer] = useState<{
    [agg_type: string]: any;
  } | null>(null);
  ...
  const [aggSelection, setAggSelection] = useState<OptionDefinition>(
    aggOptions.find(item => item.value === 'SUM')!
  );
    ...
    fetch('http://0.0.0.0:8080/api/analysis_result.json')
      ...
      .then((res: AnalysisResult) => {
        ...
        for (const [aggType, agg_metric] of aggTypes) {
          ...
          const items: TilesSizeAggByZLayer[] = (res as any)[agg_metric].map(
            (item: TilesSizeAggByZLayer) => item
          );
          ...
          for (const layer_name of Array.from(allLayers).sort()) {
            const seriesData = (res as any)[agg_metric].map((item: TilesSizeAggByZLayer) =>
              item.layers[layer_name]
                ? bytesConverted(item.layers[layer_name], unit, true, 0)
                : null
            );
              ...
              tooltip: {
                ...
                formatter: (params: any) => {
                  ...
                  const total = Math.round(
                    bytesConverted(totals.get(dataIndex) as number, unit, true, 1)
                  );
  ...
  return (
    <ContentLayout header={<Header variant="h1">Tileset Mertics</Header>}>
      <SpaceBetween direction="vertical" size="m">
        ...
        >
          {tilesSizeAggbyZLayer !== null ? (
            <ReactEcharts
              option={tilesSizeAggbyZLayer[aggSelection.value!]}
              style={CHART_STYLE}
            ></ReactEcharts>
          ) : (
            <Spinner />
